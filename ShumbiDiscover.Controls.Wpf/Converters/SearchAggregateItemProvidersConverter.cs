...
namespace ShumbiDiscover.Controls.Converters
{
    ...
    /// </summary>
    public class SearchAggregateItemProvidersConverter : IValueConverter
    {
        ...
        public static string Convert(List<SearchProviderRank> searchProviderRanks)
        {
            ...
            if (searchProviderRanks != null)
            {
                List<string> prov = new List<string>();

                for(int i = 0; i < searchProviderRanks.Count; i++)
                {
                    if (_providerFilters.Contains(searchProviderRanks[i].Provider))
                    {
                        prov.Add(searchProviderRanks[i].Provider);
                    }
                }

                if (prov.Count > 0)
                {
                    res = string.Join(", ", prov.ToArray());
                }
            }
            return (res);
        }
